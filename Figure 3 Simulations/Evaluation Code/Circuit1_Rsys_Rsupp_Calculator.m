...
for data_i = 1:length(datass)
    for kernel_i = 1:length(kers)
        ...
        exper = ['Circuit1_data', datas, '_kernel', kernels,'_sd' , seeds{sd_i}, '_nosignconstraint'];
       ...
       true_ag_bin = true_ag>0;
        ...
        norm_factor = sum(sum(sum(sum(sum(true_ba.^2)))))+ sum(sum(sum(true_ag.^2)));
        norm_factor = norm_factor+ sum(sum(sum(sum(sum(lrnd_ba.^2)))))+ sum(sum(sum(lrnd_ag.^2)));
        norm_factor = 0.5*norm_factor;
        
        
        norm_factor_bin = sum(sum(sum(sum(sum(true_ba_bin.^2)))))+ sum(sum(sum(true_ag_bin.^2))) ;
        norm_factor_bin = norm_factor_bin+ sum(sum(sum(sum(sum(lrnd_ba_bin.^2)))))+ sum(sum(sum(lrnd_ag_bin.^2))) ;
        norm_factor_bin = 0.5*norm_factor_bin;
        
        struct_proj1 = sum(sum(sum(sum(sum(true_ba1.*lrnd_ba1)))))+ sum(sum(sum(true_ag1.*lrnd_ag1)));
        
        struct_proj1 = struct_proj1/norm_factor;
        
        struct_proj1_bin = sum(sum(sum(sum(sum(true_ba1_bin.*lrnd_ba1_bin)))))+ sum(sum(sum(true_ag1_bin.*lrnd_ag1_bin)));
        ...
        freenum(data_i, kernel_i, sd_i)=agfree+bafree;
        end
    end
...
for i =1:length(kers) 
    errorbar(log10([60, 340, 1400, 5500, 22000]), mns_struct(i, :), stds_struct(i, :), '-o', 'LineWidth', 2.0, 'Color', squeeze(colors(i, :)))
...
hold on;
for i =1:length(kers)
    errorbar(log10([60, 340, 1400, 5500, 22000]), mns(i, :), stds(i, :), '-o', 'LineWidth', 2.0, 'Color', squeeze(colors(i, :)))
...
fig=figure; hold on;
for i=1:length(kers)
plot(log10([60, 340, 1400, 5500, 22000]), train_loss_all(i, :), '-o', 'LineWidth', 2.0, 'Color', squeeze(colors(i, :)));
end
...
fig=figure;hold on;
for i=1:length(kers)
plot(log10([60, 340, 1400, 5500, 22000]), test_loss_all(i, :), '-o', 'LineWidth', 2.0, 'Color', squeeze(colors(i, :)));
end
legend(num2str(round(freenumavg')))
xlabel('Training data (log(# images))')
ylabel('test loss')
